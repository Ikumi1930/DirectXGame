...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	WinAPI* winapi = new WinAPI(L"CG2");
...
//現時点でincludeはしないが、includeに対応するための設定を行っておく
		...
		*****************************/
		...
		hr = device->CreateCommittedResource(&uploadHeapProperities,D3D12_HEAP_FLAG_NONE,
			...
			*****************************/
            ...
            //上
			vertexDate[1] = { 0.0f, 0.5f ,0.01f, 1.0f };
			//右下
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*****************************
			...
			//RootSignatureを設定。PSOに設定しているけど
			commandList->SetGraphicsRootSignature(rootSignature);
			commandList->SetPipelineState(graphicsPipelineState);//PSOを設定
			commandList->IASetVertexBuffers(0, 1, &vertexBufferView);//VBVを設定
			//形状を設定。PSOに設定しているものとはまた別。同じものを設定すると考えておけばいい
			commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
			//画鋲！（DrawCall/ドローコール）。３頂点で１つのインスタンス。インスタンスについては今後
			...
			**/
			...
			//コマンドリストの内容を確定させる。すべてのコマンドえお積んでからCloseすること
