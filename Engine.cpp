...
void MyEngine::CreateRootSignature() {
	...
	for (int i = 0; i < 2; i++) {
		...
		descriptoraRange[0].OffsetInDescriptorsFromTableStart =
		    D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; // Offsetを自動計算
		rootParameters[i][2].ParameterType =
		...
		rootParameters[i][2].DescriptorTable.pDescriptorRanges =
		    descriptoraRange; // tableの中身の配列を指定
		rootParameters[i][2].DescriptorTable.NumDescriptorRanges =
		    _countof(descriptoraRange); // Tableで利用する数
	...
	rootParameters[1][4].DescriptorTable.NumDescriptorRanges =
...
void MyEngine::RasterizerState() {
	...
	vertexShaderBlob_ = CompileShader(
	    L"Object3d.VS.hlsl", L"vs_6_0", dxcUtils_[0], dxcCompiler_[0], includeHandler_[0]);
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(
	    L"Object3d.PS.hlsl", L"ps_6_0", dxcUtils_[0], dxcCompiler_[0], includeHandler_[0]);
	assert(pixelShaderBlob_ != nullptr);

	// Shaderをコンパイルする
	particleVertexShaderBlob_ = CompileShader(
	    L"./Particle.VS.hlsl", L"vs_6_0", dxcUtils_[1], dxcCompiler_[1], includeHandler_[1]);
	assert(particleVertexShaderBlob_ != nullptr);

	particlePixelShaderBlob_ = CompileShader(
	    L"Particle.PS.hlsl", L"ps_6_0", dxcUtils_[1], dxcCompiler_[1], includeHandler_[1]);
...
void MyEngine::InitializePSO() {
	for (int i = 0; i < 2; i++) {
		...
		HRESULT hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(
		    &graphicsPipelineStateDesc[i], IID_PPV_ARGS(&graphicsPipelineState_[i]));
		assert(SUCCEEDED(hr));
	}
}
...
void MyEngine::SettingDepth() {
	...
	depthStencilDesc_[0].DepthFunc =
	    D3D12_COMPARISON_FUNC_LESS_EQUAL; // 比較関数、近ければ描画される

	// 1番がパーティクル用
	depthStencilDesc_[1].DepthEnable = true;                           // 有効化
	depthStencilDesc_[1].DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO; // 書き込み
	depthStencilDesc_[1].DepthFunc =
	    D3D12_COMPARISON_FUNC_LESS_EQUAL; // 比較関数、近ければ描画される
}

void MyEngine::Initialize(const wchar_t* title, int32_t width, int32_t height) {
	dxCommon_ = new DirectXCommon();
	dxCommon_->Initialization(
	    title, WinApp::GetInstance()->kClientWidth, WinApp::GetInstance()->kClientHeight);

	descriptorSizeDSV =
	    dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
	descriptorSizeRTV =
	    dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
	descriptorSizeSRV = dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(
...
Microsoft::WRL::ComPtr<ID3D12Resource> MyEngine::CreateTextureResource(
	...
	resourceDesc.Dimension =
	    D3D12_RESOURCE_DIMENSION(metadata.dimension); // Textureの次元数。普段は2次元
	...
	HRESULT hr = device->CreateCommittedResource(
	    &heapProperties, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_COPY_DEST,
...
[[nodiscard]] Microsoft::WRL::ComPtr<ID3D12Resource> MyEngine::UploadtextureData(
    Microsoft::WRL::ComPtr<ID3D12Resource> texture, const DirectX::ScratchImage& mipImages,
    uint32_t index) {
	std::vector<D3D12_SUBRESOURCE_DATA> subresource;
	DirectX::PrepareUpload(
	    dxCommon_->GetDevice().Get(), mipImages.GetImages(), mipImages.GetImageCount(),
	    mipImages.GetMetadata(), subresource);
	uint64_t intermediateSize =
	    GetRequiredIntermediateSize(texture.Get(), 0, UINT(subresource.size()));
	intermediateResource_[index] =
	    dxCommon_->CreateBufferResource(dxCommon_->GetDevice(), intermediateSize);
	UpdateSubresources(
	    dxCommon_->GetCommandList().Get(), texture.Get(), intermediateResource_[index].Get(), 0, 0,
...
void MyEngine::SettingTexture(const std::string& filePath, uint32_t index) {
	...
	textureSrvHandleGPU_[index] =
	    GetGPUDescriptorHandle(dxCommon_->GetSrvDescriptiorHeap(), descriptorSizeSRV, index);
	textureSrvHandleCPU_[index] =
	    GetCPUDescriptorHandle(dxCommon_->GetSrvDescriptiorHeap(), descriptorSizeSRV, index);

	// 先頭はIMGUIが使ってるので、その次を使う
	/*textureSrvHandleCPU_[index].ptr +=
	dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	textureSrvHandleGPU_[index].ptr +=
	dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);*/

	// SRVの生成
	dxCommon_->GetDevice()->CreateShaderResourceView(
	    textureResource_[index].Get(), &srvDesc, textureSrvHandleCPU_[index]);
}
...
void MyEngine::SettingObjTexture(uint32_t index) {
	...
	textureResource_[index] = CreateTextureResource(dxCommon_->GetDevice(), metadata);
	intermediateResource_[index] =
	    UploadtextureData(textureResource_[index].Get(), mipImages2, index);
	...
	// 先頭はIMGUIが使ってるので、その次を使う
	textureSrvHandleCPU_[index].ptr += dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(
	    D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	textureSrvHandleGPU_[index].ptr += dxCommon_->GetDevice()->GetDescriptorHandleIncrementSize(
	    D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
D3D12_CPU_DESCRIPTOR_HANDLE MyEngine::GetCPUDescriptorHandle(
    Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> descriptorheap, uint32_t descriptorSize,
    uint32_t index) {
...
D3D12_GPU_DESCRIPTOR_HANDLE MyEngine::GetGPUDescriptorHandle(
    Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> descriptorheap, uint32_t descriptorSize,
    uint32_t index) {
...
MaterialData MyEngine::LoadMaterialTemplateFile(
