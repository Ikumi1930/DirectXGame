...
public:
	...
	MaterialData
	    LoadMaterialTemplateFile(const std::string& directoryPath, const std::string& filename);
	...
	Microsoft::WRL::ComPtr<ID3D12PipelineState> GetGraphicsPipelineState() {
		return graphicsPipelineState_[0];
	}

	Microsoft::WRL::ComPtr<ID3D12RootSignature> GetRootSignature2() { return rootSignature_[1]; }

	Microsoft::WRL::ComPtr<ID3D12PipelineState> GetGraphicsPipelineState2() {
		return graphicsPipelineState_[1];
	}
