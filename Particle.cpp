...
void Particle::Initialize(
    DirectXCommon* dxCommon, MyEngine* engine, const std::string& directoryPath,
...
void Particle::Draw(ParticleData* transforms, uint32_t index, const Transform& cameraTransform) {
	...
	for (uint32_t index = 0; index < kNumInstance; ++index) {
		...
		Matrix4x4 worldMatrix = MakeAffineMatrix(
		    transforms[index].transform.scale, transforms[index].transform.rotate,
		    transforms[index].transform.translate);
		Matrix4x4 cameraMatrix = MakeAffineMatrix(
		    cameraTransform.scale, cameraTransform.rotate, cameraTransform.translate);
		Matrix4x4 viewMatrix = Inverse(cameraMatrix);
		Matrix4x4 projectionMatrix = MakePerspectiveFovMatrix(
		    0.45f,
		    float(dxCommon_->GetWin()->kClientWidth) / float(dxCommon_->GetWin()->kClientHeight),
		    0.1f, 100.0f);
...
void Particle::SettingVertex() {
	...
	vertexResource = dxCommon_->CreateBufferResource(
	    dxCommon_->GetDevice(), sizeof(VertexData) * modelData.vertices.size());
	// リソースの先頭のアドレスから使う
...
void Particle::TransformMatrix() {
	wvpResource_ = dxCommon_->CreateBufferResource(
	    dxCommon_->GetDevice(), sizeof(ParticleForGPU) * kNumInstance);
...
void Particle::SettingInstance() {
	...
	instancingSrvHandleCPU_ = engine_->GetCPUDescriptorHandle(
	    dxCommon_->GetSrvDescriptiorHeap(), engine_->GetdescriptorSizeSRV(), 1);
	instancingSrvHandleGPU_ = engine_->GetGPUDescriptorHandle(
	    dxCommon_->GetSrvDescriptiorHeap(), engine_->GetdescriptorSizeSRV(), 1);

	dxCommon_->GetDevice()->CreateShaderResourceView(
	    instancingResource.Get(), &instancingSrvDesc, instancingSrvHandleCPU_);
}
...
ParticleData Particle::MakeNewParticle(std::mt19937& randomEngine) {
	...
	particle.speed = {
	    distribution(randomEngine), distribution(randomEngine), distribution(randomEngine)};
	particle.color = {
	    distColor(randomEngine), distColor(randomEngine), distColor(randomEngine), 1.0f};
