...
void DirectXCommon::Initialization(
    const wchar_t* title, int32_t backBufferWidth, int32_t backBufferHeight) {
...
void DirectXCommon::ImGuiInitialize() {
	...
	ImGui_ImplDX12_Init(
	    device_.Get(), swapChainDesc_.BufferCount, rtvDesc_.Format, srvDescriptorHeap_.Get(),
...
void DirectXCommon::CreateSwapChain() {
	...
	hr_ = dxgiFactory_->CreateSwapChainForHwnd(
	    commandQueue_.Get(), WinApp::GetInstance()->GetHwnd(), &swapChainDesc_, nullptr, nullptr,
	...
	// SRV用ディスクリプタヒープの生成
	srvDescriptorHeap_ =
	    CreateDescriptorHeap(device_, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 128, true);
}
...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> DirectXCommon::CreateDescriptorHeap(
	...
	descriptionHeapDesc.NumDescriptors =
	    numDescriptors; // ダブルバッファ用に二つ。多くても別にかまわない
	descriptionHeapDesc.Flags =
...
void DirectXCommon::CreateDepthStensil() {
	depthStencilResource_ = CreateDepthStenciltextureResource(
	    device_.Get(), WinApp::kClientWidth, WinApp::kClientHeight);
