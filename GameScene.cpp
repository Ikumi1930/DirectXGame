...
void GameScene::Update() {
	...
	for (int i = 0; i < 2; i++) {
		...
		//}
		worldMatrix_ =
		    MakeAffineMatrix(transform_[i].scale, transform_[i].rotate, transform_[i].translate);
	}
	...
	sphereMatrix_ = MakeAffineMatrix(
	    sphereTransform_.scale, sphereTransform_.rotate, sphereTransform_.translate);

	Matrix4x4 sphereAffine = MakeAffineMatrix(
	    sphereTransform_.scale, sphereTransform_.rotate, sphereTransform_.translate);
	Matrix4x4 cameraMatrix = MakeAffineMatrix(
	    cameraTransform_.scale, cameraTransform_.rotate, cameraTransform_.translate);
	Matrix4x4 viewMatrix = Inverse(cameraMatrix);
	Matrix4x4 projectionMatrix = MakePerspectiveFovMatrix(
	    0.45f, float(dxCommon_->GetWin()->kClientWidth) / float(dxCommon_->GetWin()->kClientHeight),
	    0.1f, 100.0f);
	Matrix4x4 worldViewProjectionMatrix =
	    Multiply(worldMatrix_, Multiply(viewMatrix, projectionMatrix));
	...
	if (ImGui::TreeNode("Sphere")) {
		...
		ImGui::DragFloat2(
		    "UVTranslate", &sphere_->uvTransformSprite.translate.x, 0.01f, -10.0f, 10.0f);
		ImGui::DragFloat2(
		    "UVScale", &sphere_->uvTransformSprite.scale.x, 0.01f, -10.0f, 10.0f);
	...
	if (ImGui::TreeNode("Object")) {
		...
		ImGui::DragFloat4("LightColor", &directionalLight_.color.x, 1.0f);
		ImGui::DragFloat3("DirectionLight", &directionalLight_.direction.x, 0.1f);
		/*ImGui::DragFloat2("UVTranslate", &sphere_->uvTransformSprite.translate.x, 0.01f,
		-10.0f, 10.0f); ImGui::DragFloat2("UVScale", &sphere_->uvTransformSprite.scale.x, 0.01f,
		-10.0f, 10.0f); ImGui::SliderAngle("UVRotate", &sphere_->uvTransformSprite.rotate.z);*/
	...
	if (ImGui::TreeNode("Sprite")) {
		...
		for (int i = 0; i < 2; i++) {
			ImGui::DragFloat2(
			    "UVTranslate", &sprite_[i]->uvTransformSprite.translate.x, 0.01f, -10.0f,
			    10.0f);
			ImGui::DragFloat2(
			    "UVScale", &sprite_[i]->uvTransformSprite.scale.x, 0.01f, -10.0f, 10.0f);
			ImGui::SliderAngle("UVRotate", &sprite_[i]->uvTransformSprite.rotate.x);
		}
		ImGui::TreePop();
	}
...
void GameScene::Draw() {
	...
	if (triangleDrawA_) {
		triangle_[0]->Draw(
		    triangleData_[0].position[0], triangleData_[0].position[1],
		    triangleData_[0].position[2], triangleData_[0].material, transform_[0],
		    cameraTransform_, 2, directionalLight_);
	}

	if (triangleDrawB_) {
		triangle_[1]->Draw(
		    triangleData_[1].position[0], triangleData_[1].position[1],
		    triangleData_[1].position[2], triangleData_[1].material, transform_[1],
		    cameraTransform_, 2, directionalLight_);
	}
	...
	if (spriteDraw_) {
		for (int i = 0; i < 1; i++) {
			sprite_[i]->Draw(
			    ...
			    spriteData_.material, 2, directionalLight_);
		}
	}
	if (objectDraw_) {
		for (int i = 0; i < 2; i++) {
			object_[i]->Draw(
			    objectMaterial_[i], objectTransform_[i], 3, cameraTransform_, directionalLight_,
			    true);
		}
	}
